...
from model.DDSF_Net import DDSF_Net
import argparse
...
torch.cuda.manual_seed_all(3208)
...
model_restored = DDSF_Net(inp_channels=3,out_channels=3,dim = 16,num_blocks = [2,4,8,16],num_refinement_blocks = 2,heads = [1,2,4,8],ffn_expansion_factor = 2.66,bias = False,LayerNorm_type = 'WithBias',attention=True,skip = False)
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        # Compute loss
        loss = (char_loss_fn(restored, target)
        +ssim_loss_fn(restored, target)
        +vgg_loss_fn(restored, target)
        + lambda_edge * edge_loss_fn(restored, target))
